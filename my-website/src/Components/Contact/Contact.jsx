...
//#####################################################################################
    ...
    const handleSubmit = (event) => {
        ...
        if (!isCaptchaConfirmed) {
            ...
            return; // Stop form submission if reCAPTCHA is not confirmed
        }
    ...
    const getData = async (e) => {
        ...
        if (!isFormValid) {
            toast.error('Please fill in all fields correctly before submitting the form.', {
                position: "top-right",
                autoClose: 5000,
        ...
        // Check if ReCAPTCHA is confirmed
        if (!isCaptchaConfirmed) {
            toast.error('Please confirm the ReCAPTCHA before submitting the form.', {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
            ...
            <motion.div className="contact-contents" variants={variants}>
                ...
                <div className="formContainer">
                    ...
                    <motion.form onSubmit={handleSubmit} method="POST" initial={{opacity:0}} whileInView={{opacity:1}} transition={{delay:4, duration:1}} action={FORM_ENDPOINT}>
                        {/* Input fields */}
                        <input type="text" name="FullName" placeholder="Full Names"   className={!fullNameValid ? "invalid" : ""} 
                        value={user.FullName} onChange={data}
                        onFocus={() => setFullNameValid(true)}/>
                        {!fullNameValid && <p className="error">{fullNameError}</p>}

                        <input type="email" name="Email"  placeholder="Email"  className={!emailValid ? "invalid" : ""}
                        value={user.Email} onChange={data}
                        onFocus={() => setEmailValid(true)}/>
                        {!emailValid && <p className="error">{emailError}</p>}

                        <textarea name="Message" id="" cols="30" rows="10"placeholder="Message"  className={!messageValid ? "invalid" : ""}
                        value={user.Message} onChange={data}
                        onFocus={() => setMessageValid(true)}></textarea>
